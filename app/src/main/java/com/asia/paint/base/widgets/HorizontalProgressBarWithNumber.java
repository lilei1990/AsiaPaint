...
public class HorizontalProgressBarWithNumber extends ProgressBar {
    ...
    // 圆的颜色
    protected int intmCircleColor = DEFAULT_CIRCLE_COLOR;
    protected int outmCircleColor = DEFAULT_CIRCLE_COLOR;
    ...
    private void obtainStyledAttributes(AttributeSet attrs) {
        ...
        intmCircleColor = attributes.getColor(R.styleable.HorizontalProgressBarWithNumber_progress_circlein_color, DEFAULT_CIRCLE_COLOR);
        outmCircleColor = attributes.getColor(R.styleable.HorizontalProgressBarWithNumber_progress_circleout_color, DEFAULT_CIRCLE_COLOR);
        ...
        attributes.recycle();
        int left = (int) (mReachedProgressBarHeight * 1.5), right = (int) (mReachedProgressBarHeight * 1.5);
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        if (mIfDrawText) {
            mPaint.setColor(mTextColor);
            canvas.drawText(text, progressPosX - textWidth / 2, mReachedProgressBarHeight*2+10, mPaint);
        }
//        // 圆
//        if (mIfDrawCircle) {
...
//     //绘制外圆
        this.mPaint.setColor(outmCircleColor);
        this.mPaint.setStrokeWidth(radius/2);
        canvas.drawCircle(progressPosX,0, radius+10, this.mPaint);
        //绘制圆环,设置圆环的颜色修改画笔的颜色
        this.mPaint.setColor(intmCircleColor);
        this.mPaint.setStrokeWidth(radius/2);//设置圆环宽度
        canvas.drawCircle(progressPosX,0, radius/2, this.mPaint);//圆环宽度为中间圆
        canvas.restore();
    }
    /**
     * dp 2 px
