...
//                    addCart(spec.spec_id, count);
    ...
    public CallbackDate<UpdateStatusBean> updateStatus(String url) {
        NetworkFactory.createService(ShopService.class)
                .upStatus(url)
                .compose(new NetworkObservableTransformer<>())
                .subscribe(new DefaultNetworkObserver<UpdateStatusBean>(false) {
                    @Override
                    public void onSubscribe(Disposable d) {
                        addDisposable(d);
                    }
                    ...
                    @Override
                    public void onResponse(UpdateStatusBean response) {
                        statusdata.setData(response);
                    }
